// try to use StringBuffer instead of string concatenation to improve performance
 ...
 printOn = function(x, ws){
    if (x === undefined || x === null)ws.nextPutAll("" + x);
 ...
 objectThatDelegatesTo = function(x, props){
    ...
    f.prototype = x;
    var r = new f();
    for (var p in props)if (props.hasOwnProperty(p))r[p] = props[p];
    return r;
};// some reflective stuff
 
 ownPropertyNames = function(x){
    var r =[];
    for (var name in x)if (x.hasOwnProperty(name))r.push(name);
    return r;
};
 ...
 Array.prototype.map = function(f){
    var r =[];
    for (var idx = 0;idx < this.length;idx++)r[idx] = f(this[idx]);
    return r;
};
 
 Array.prototype.reduce = function(f, z){
    var r = z;
    for (var idx = 0;idx < this.length;idx++)r = f(r, this[idx]);
    return r;
};
 ...
 String.prototype.pad = function(s, len){
    var r = this;
    while (r.length < len)r = s + r;
    return r;
};
 escapeStringFor = new Object();
 for (var c = 0;c < 128;c++)escapeStringFor[c] = String.fromCharCode(c);
 ...
 escapeChar = function(c){
    ...
    if (charCode < 128)return escapeStringFor[charCode];
    else if (128 <= charCode && charCode < 256)return "\\x" + charCode.toString(16).pad("0", 2);
    else return "\\u" + charCode.toString(16).pad("0", 4);
};
 ...
 String.prototype.toProgramString = function(){
    var ws = '"'.writeStream();
    for (var idx = 0;idx < this.length;idx++)ws.nextPutAll(escapeChar(this.charAt(idx)));
    ws.nextPutAll('"');
    return ws.contents();
 ...
 tempnam.n = 0;// unique tags for objects (useful for making "hash tables")
